...
class BaseColocResultFilter(FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.SIGNAL_ANNOTATIONS = {
            "primary_signal_trait": (
                "signal1__analysis__trait__uuid",
                "signal2__analysis__trait__uuid",
            ),
            "secondary_signal_trait": (
                "signal2__analysis__trait__uuid",
                "signal1__analysis__trait__uuid",
            ),
            "primary_signal_trait_label": (
                "signal1__analysis__trait__label",
                "signal2__analysis__trait__label",
            ),
            "secondary_signal_trait_label": (
                "signal2__analysis__trait__label",
                "signal1__analysis__trait__label",
            ),
            "primary_signal_chrom": (
                "signal1__lead_variant__chrom",
                "signal2__lead_variant__chrom",
            ),
            "secondary_signal_chrom": (
                "signal2__lead_variant__chrom",
                "signal1__lead_variant__chrom",
            ),
            "primary_signal_pos": (
                "signal1__lead_variant__pos",
                "signal2__lead_variant__pos",
            ),
            "secondary_signal_pos": (
                "signal2__lead_variant__pos",
                "signal1__lead_variant__pos",
            ),
            "primary_signal_logp": ("signal1__neg_log_p", "signal2__neg_log_p"),
            "secondary_signal_logp": ("signal2__neg_log_p", "signal1__neg_log_p"),
            "primary_signal_tissue": (
                "signal1__analysis__tissue",
                "signal2__analysis__tissue",
            ),
            "secondary_signal_tissue": (
                "signal2__analysis__tissue",
                "signal1__analysis__tissue",
            ),
            "primary_signal_cell_type": (
                "signal1__analysis__cell_type",
                "signal2__analysis__cell_type",
            ),
            "secondary_signal_cell_type": (
                "signal2__analysis__cell_type",
                "signal1__analysis__cell_type",
            ),
            "primary_signal_study": (
                "signal1__analysis__study__uuid",
                "signal2__analysis__study__uuid",
            ),
            "secondary_signal_study": (
                "signal2__analysis__study__uuid",
                "signal1__analysis__study__uuid",
            ),
            "primary_signal_gene_ens_id": (
                "signal1__analysis__trait__gene__ens_id",
                "signal2__analysis__trait__gene__ens_id",
            ),
            "secondary_signal_gene_ens_id": (
                "signal2__analysis__trait__gene__ens_id",
                "signal1__analysis__trait__gene__ens_id",
            ),
            "primary_signal_gene_symbol": (
                "signal1__analysis__trait__gene__symbol",
                "signal2__analysis__trait__gene__symbol",
            ),
            "secondary_signal_gene_symbol": (
                "signal2__analysis__trait__gene__symbol",
                "signal1__analysis__trait__gene__symbol",
            ),
            "primary_signal_exon_ens_id": (
                "signal1__analysis__trait__exon__ens_id",
                "signal2__analysis__trait__exon__ens_id",
            ),
            "secondary_signal_exon_ens_id": (
                "signal2__analysis__trait__exon__ens_id",
                "signal1__analysis__trait__exon__ens_id",
            ),
        }
        ...
        for field in ordering_param.split(","):
            ...
            for internal_name, (_, _) in self.SIGNAL_ANNOTATIONS.items():
                ...
                if field in (
                    ...
                    internal = field.replace("signal1_", "primary_signal_").replace(
                        "signal2_", "secondary_signal_"
                    )
    ...
    def filter_queryset(self, queryset):
            ...
            queryset = queryset.annotate(
                **{
                    field_name: Greatest(
                        Case(
                            When(
                                signal1__analysis__analysis_type=analysis_type_name,
                                then=F("signal1__neg_log_p"),
                            ),
                            default=Value(float("-inf")),
                        ),
                        Case(
                            When(
                                signal2__analysis__analysis_type=analysis_type_name,
                                then=F("signal2__neg_log_p"),
                            ),
                            default=Value(float("-inf")),
                        ),
                    )
        ...
        for field_name, (
            swap_false_path,
            swap_true_path,
        ) in self.SIGNAL_ANNOTATIONS.items():
    ...
    signal1_analysis = CharFilter(
        field_name="signal1__analysis__uuid",
        lookup_expr="exact",
        label="Signal 1 analysis UUID",
    )
    signal2_analysis = CharFilter(
        field_name="signal2__analysis__uuid",
        lookup_expr="exact",
        label="Signal 2 analysis UUID",
    )

    signal1_trait = CharFilter(
        method="filter_signal1_trait", lookup_expr="exact", label="Signal 1 trait UUID"
    )
    ...
    signal2_trait = CharFilter(
        method="filter_signal2_trait", lookup_expr="exact", label="Signal 2 trait UUID"
    )
    ...
    signal1_min_logp = NumberFilter(
        field_name="signal1__neg_log_p",
        lookup_expr="gte",
        label="Minimum -log10 p-value for signal 1",
    )
    signal2_min_logp = NumberFilter(
        field_name="signal2__neg_log_p",
        lookup_expr="gte",
...
class FinemappedSignalResultFilter(FilterSet):
    ...
    min_logp = NumberFilter(
        field_name="neg_log_p", lookup_expr="gte", label="Minimum -log10 p-value"
    )
